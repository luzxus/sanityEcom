...
export const fetchProductsByCategory = async (category: string) => {
  const query = `*[_type == "product" && category->slug.current == $fpc] {
    name,
    slug,
    price,
    details,
    ...
    "bannerImage": category->banner.asset -> url,
    "tags": tags[]->name
  }`

  const params = { fpc: category } // Define parameters object

  const result = await client.fetch(query, params) // Pass parameters to the fetch method

  const bannerQuery = `*[_type == "banner" && category._ref in *[_type == "category" && slug.current == $fpc]._id][0]{
    buttonText,
      saleTime,
      largeText1,
      discount,
...
export const fetchProductsByTags = async (category: string) => {
  ...
  const query = `*[_type == "product" && $fpt in tags[] -> slug.current] {
    ...,
    "tags": tags[]->slug.current
  }
  `

  const params = { fpt: category } // Define parameters object
